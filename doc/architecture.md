...
- **Archive Processing**: `archiver` stages ZIP output in Vercel's ephemeral filesystem, then streams the completed file to the client without holding the archive in process memory.
...
3. **Download (`/api/export/download`)**: Client uses the exact tokenized `downloadUrl` returned by request/status. The server reads the requesting user’s Firestore profile, emergency configuration, and rides before sending headers, stages the generated ZIP in Vercel's ephemeral filesystem, and then streams the completed file. Firestore/GPX/archive failures return structured `502` JSON before a ZIP response begins. The six-hour retention marker is persisted only after the response finishes; an untouched or interrupted record remains available until its existing expiry window.
